...
const createUser = async (req, res) => {
  ...
  try {
    ...
    const result = await client.query(
      ...
      await postAuditing(audit_operation,audit_affected_table,user_id,audit_field_affect,changes);
...
const updateUser = async (req, res) => {
  ...
  try {
    const user_preview = await client.query("select*from users where user_id = $1",[userId])
    const user_full_name = `${user_first_name} ${user_last_name}`;
    const result = await client.query(
      `
